...
subtitle: "Data Wrangle and Exploratory Analyses, Part I"
author: "Chris Guo"
...
The point of this document is to make open and share-able the research methods used for my dissertation on nearshore fish communities in Alaska working towards a PhD in marine biology at the University of Alaska Fairbanks. Here, I cover most steps of the initial data preparation for my second and third chapters concerning spatial and temporal distributions of nearshore fishes across the state. This and other project files can be accessed at this [github repo](https://github.com/kbnerr/nearshore-fish). Following this document, Part II of the data wrangling and exploratory analyses can be accessed at <https://rpubs.com/chguo1/1232673>.

In this document I show the steps taken in wrangling the NOAA Nearshore Fish Atlas (NFA) database. This replaces the separate scripts (named "./FishAtlas\_#\_description.R") and combines them into a single, streamlined R markdown file. These R scripts will be archived in case I need to replicate older methods. Namely, I used to use **rgdal** package for some steps in the process but that package stopped being supported around Oct 2023, moving my workflow to the **sf** package.
...
## Taxa lists - before edits {.tabset .tabset-pills}
...
### Summary of Juvenile and Unidentified {.unnumbered}
...
I do not show results of the below code. See comments for details of the steps taken.
...
# Fix the "or" cases in Sp_ScientificName
  ...
  mutate(Sp_ScientificName = ifelse(str_detect(Sp_ScientificName, pattern = "Anisarchus medius or Lumpenus fabricii"), Fam_ScientificName, Sp_ScientificName),
         ...
         # For cases where "species" is in Sp_ScientificName- use the genus name only
         Sp_ScientificName = ifelse(str_detect(Sp_ScientificName, pattern = "species"), Gen_ScientificName, Sp_ScientificName))
...
Dealing with rare species is common in community data analysis because of the outlier effect they have when describing the community as a whole. Depending on our research questions, we may not consider rare cases functional members of the community- in these cases it may make more sense to view the community as those taxa most well represented. But if I were more interested in richness, then I would want to conservatively retain rare taxa.

We'll want to be objective in how we define rare taxa. It makes sense to me that occurrence holds more weight than abundance; although, I think we can convince ourselves that removing single count and even single occurrence species would be reasonable. Since we do not have any information to let us know whether those cases represent an actual community member or whether they represent some sort of sampling effect (e.g., imagine a project had an ID'er that was an expert in juvenile salmon). Our decision should balance the information lost vs retained, acknowledging that this differs at different taxa levels. To begin analyses, we'll want to try using the lowest taxonomy possible (species), but it'll be good practice to define "rare"  ap priori so we're not too biased after conducting EDA. For now let's say that taxa with <0.25% occurrence are rare (<`r ceiling(n_distinct(events$VisitID) * 0.0025)` samples), which causes:

- `r round(n_distinct(filter(fam.freq.occur, Perc_Occurrence < 0.25)) / nrow(fam.freq.occur) * 100, 1)`% reduction in richness at the Family level

- `r round(n_distinct(filter(gen.freq.occur, Perc_Occurrence < 0.25)) / nrow(gen.freq.occur) * 100, 1)`% reduction in richness at the Genus level

- `r round(n_distinct(filter(sp.freq.occur, Perc_Occurrence < 0.25)) / nrow(sp.freq.occur) * 100, 1)`% reduction in richness at the Species Level

Note that this fixed cut-off affects species diversity more so than family diversity. Some of this can be explained by the fact that the species list contains many identifications at the genus and family level, which makes sense since identification often includes cases where researchers aggregate 'iffy' ID's into a higher class. I expect this issue to be especially apparent in a collective dataset like the NFA.
...
# Table of rare species abundance and occurrence
...
## Aggregating rare species to genus and family {.tabset .tabset-pills}

Looking at the rare species list, I see a some rare taxa that could be aggregated into higher taxonomies. There are Liparis and Sebastes species that only occur in one sample, but their genus is not rare. These two types of fish are notoriously difficult to identify and could have resulted from certain projects benefiting from species expertise that others did not have. Xiphister species also has this characteristic. Another case I want to point out- there are rare occurrences of Bathymaster identified to species and also those identified to the family, Bathymasteridae). In addition to Bathymasteridae, I see rare species names that are already at the genus level (e.g., Citharchthys) or family level (Zoarcidae). Similarly, Pholis fasciata is a rare species but if we look at the full list of species names, we see that there is a non-rare occurrence of data classified as Pholidae. It is likely we'll need to remove rare taxa somehow, but we also want avoid losing information that can be captured by using the genus or family name especially in cases where it is already occurring (e.g., Pholidae).

So we have a two step aggregation process. For species considered rare (occurring in <`r ceiling(n_distinct(events$VisitID) * 0.0025)` samples), I want to replace their name with genus name. Those that are already listed as a family name would stay at the family level. After that if taxa are still rare, then we aggregate again and replace with family name. Those taxa still rare after this second round of aggregation have much less information to contribute and are good candidates for removing from the data.

### Rare species after aggregating to genera {.unnumbered}

```{r echo = FALSE}
# Create an vector of all of our rare species as originally labelled
rare.species.1 = rare.sp$Sp_ScientificName %>% as.vector()

# Replace rare species with genera names
catch.3a = mutate(catch.3, Sp_ScientificName = ifelse(Sp_ScientificName %in% rare.species.1, Gen_ScientificName, Sp_ScientificName))

# Recalculate species abundance, first aggregation
sp.abun.1 = catch.3a %>%
  summarise(Abundance = sum(Count), .by = Sp_ScientificName) %>%
  mutate(Sp_ScientificName = fct_reorder(as.factor(Sp_ScientificName), desc(Abundance)))
# Recalculate species frequency of occurrence, first aggregation
sp.freq.occur.1 = left_join(catch.3a, select(events, VisitID, EventID), by = "EventID") %>%
  summarise(Presence = n_distinct(Sp_ScientificName), .by = c(VisitID, Sp_ScientificName)) %>%
  summarise(Occurrence = sum(Presence), .by = Sp_ScientificName) %>%
  mutate(Perc_Occurrence = round(Occurrence / n_distinct(events$VisitID) * 100, 2),
         Sp_ScientificName = fct_reorder(as.factor(Sp_ScientificName), desc(Perc_Occurrence)))
# Subset rare species, first aggregation
rare.sp.gen = full_join(sp.abun.1, sp.freq.occur.1, by = 'Sp_ScientificName') %>%
  filter(Perc_Occurrence < 0.25) %>%
  select(Sp_ScientificName, Perc_Occurrence, Abundance)

# Create a table of rare species abundance and occurrence, first aggregation
rename(rare.sp.gen, `Scientific Name` = Sp_ScientificName, `Percent Occurrence` = Perc_Occurrence) %>%
  arrange(as.character(`Scientific Name`)) %>%
  datatable(rownames = FALSE,
            extensions = 'Scroller',
            options = list(deferRender = TRUE,
                           ...
                           scroller = TRUE))
```
### Rare species after aggregating to to family {.unnumbered}

```{r echo = FALSE}
# Create another vector of all of our rare species after first aggregation
rare.species.2 = rare.sp.gen$Sp_ScientificName %>% as.vector()

# Replace rare species with family names
catch.3b = mutate(catch.3a, Sp_ScientificName = ifelse(Sp_ScientificName %in% rare.species.2, Fam_ScientificName, Sp_ScientificName))

# Recalculate species abundance, second aggregation
sp.abun.2 = catch.3b %>%
  summarise(Abundance = sum(Count), .by = Sp_ScientificName) %>%
  mutate(Sp_ScientificName = fct_reorder(as.factor(Sp_ScientificName), desc(Abundance)))
# Recalculate species frequency of occurrence, second aggregation
sp.freq.occur.2 = left_join(catch.3b, select(events, VisitID, EventID), by = "EventID") %>%
  summarise(Presence = n_distinct(Sp_ScientificName), .by = c(VisitID, Sp_ScientificName)) %>%
  summarise(Occurrence = sum(Presence), .by = Sp_ScientificName) %>%
  mutate(Perc_Occurrence = round(Occurrence / n_distinct(events$VisitID) * 100, 2),
         Sp_ScientificName = fct_reorder(as.factor(Sp_ScientificName), desc(Perc_Occurrence)))
# Subset rare species, second aggregation
rare.sp.2 = full_join(sp.abun.2, sp.freq.occur.2, by = 'Sp_ScientificName') %>%
  filter(Perc_Occurrence < 0.25) %>%
  select(Sp_ScientificName, Perc_Occurrence, Abundance)

# Create a table of rare species abundance and occurrence, first aggregation
rename(rare.sp.2, `Scientific Name` = Sp_ScientificName, `Percent Occurrence` = Perc_Occurrence) %>%
  arrange(as.character(`Scientific Name`)) %>%
  datatable(rownames = FALSE,
            extensions = 'Scroller',
            options = list(deferRender = TRUE,
...
## Species list - after all edits {.tabset .tabset-pills}

### Species {.unnumbered}

```{r echo = FALSE}
# Create species table using edited catch data
sp.tbl = full_join(sp.abun.2, sp.freq.occur.2, by = 'Sp_ScientificName') %>%
...
```
We use to have `r nrow(sp.abun)` taxa at the species level, and now we have `r nrow(sp.tbl)`. We ended up with 13 rare cases after aggregating species to genus then aggregating again to family. Our most abundant rare case is Gasterosteidae (sticklebacks) with 204 individuals caught among 3 samples, which is surprising since I wouldn't think sticklebacks as difficult to ID in Alaska. The other rare cases are much less abundant and do not provide much information since they were only caught in a few samples and could only be identified to the family level. Our new object *sp.tbl* contains a variable classifying whether taxa are rare or not. This will make it easy for us to subset the data down the road if required.

Since we lost some information at lower taxonomic levels, I'll retain an object that contains the original taxonomic data (species level) in case we want to take a look at it later.

## Clean up

```{r}
# Save rare species info as .rds in our data folder
saveRDS(rare.sp, file = file.path(dir.data, "nfa_rarespecies.rds"))

# Re-assign working version of catch to a cleaner name
catch = catch.3b

# Clean up our environment for the next step:
rm(list = ls()[!ls() %in% c('data', 'events', 'catch', 'sp.tbl')])
...
## Re-creating catch summary {.tabset .tabset-pills}

We'll need to redo the abundance and occurrence calculations because the number of visits has changed. Rare taxa will still be equivalent to <0.25% occurrence. The frequency and abundance plots are not all that different. I show an updated species table below, note that I highlight rare taxa in orange.
...
## Re-summarize catch
# Abundance
sp.abun = summarise(catch, Abundance = sum(Count), .by = c(Sp_ScientificName))
...
# Save what's left,
save(list = ls(), file = file.path(here(), "data", "nfaa_1.rda"))
```
