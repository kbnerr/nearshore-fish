...
Now let's add Region to our visits df, and show it on a map. Since we're manipulating our data objects again, let's also rename visits so that we can keep track of our changes. 

```{r}
# Call our wrangled df version visits.0 and remove the non-version df from the env,
assign("visits.0", visits); rm(visits)

# Join Region by VisitID,
visits.1 = regions %>%
