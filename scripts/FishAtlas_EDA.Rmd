...
times %>%
  ...
  geom_col() +
  geom_text(aes(label = n), vjust = -0.5, color = "blue") +
  labs(x = NULL, y = NULL, title = "No. samples per Year w/o 1976")
...
```{r}
# Over all years,
left_join(times, select(visits.1, VisitID, Region), by = "VisitID") %>%
  ...
  stat_summary(geom = "text", vjust = -2, color = "black",
               fun.data = function(x) {
