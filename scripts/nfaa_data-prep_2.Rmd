...
diversity = abun %>%
  ...
  summarise(S = specnumber(Sp_ScientificName), # Richness
            ...
            J = diversity(Abundance, index = "shannon") / log(q0)) # Pielou's evenness
```
...
```{r}
regions.all = visits$Region %>% as.character() %>% unique()
regions.goa = str_subset(regions.all, "GOA|Aleutians")

# q0 (richness)
left_join(visits, diversity, by = "VisitID") %>%
  filter(Region %in% regions.goa) %>%
  ggplot() +
  aes(x = Lon, y = q0) +
  geom_point() +
...
left_join(visits, diversity, by = "VisitID") %>%
  boxplot(q0 ~ Replicates, data = .)

# remove replicates = 9, 10, 12 bc of single obs
data = left_join(visits, diversity, by = "VisitID") %>%
  mutate(Replicates = as.numeric(Replicates)) %>%
  filter(Replicates < 9)

# analysis of variance
aov.q0.reps = aov(q0 ~ as.factor(Replicates), data = data)
summary(aov.q0.reps)
# pairwise test
TukeyHSD(aov.q0.reps)
# residuals
residuals = residuals(aov.q0.reps)
plot(residuals)
plot(aov.q0.reps)
...
# kruskal-wallis test
kruskal.test(q0 ~ Replicates, data = data)

# one way t-test not assuming equal variances
oneway.test(q0 ~ Replicates, data = data)
# pairwise test
results = pairwise.t.test(data$q0, data$Replicates,
                          p.adjust.method = "BH",
                          pool.sd = FALSE)
results
...
# plot t.test results
ggplot(data) +
  aes(x = Replicates, y = q0, group = Replicates) +
  geom_boxplot() +
...
species_plots = function (abun, size, visits) {
  # Create a vector contaiing all the species names
