...
# Aggregate replicates 7 and above
richness %>%
...
# Load
weights = readRDS(file.path(dir.data, "nfaa_weights.rds"))
```

### Linking weights with samples

```{r}
# Start with the richness df
visits.1 = richness %>%
  # Re-factor replicates based on our analyses
  mutate(f.R = ifelse(Replicates > 2, "3+", Replicates) %>% as.ordered()) %>%
  # Add sample weights
  left_join(weights, by = join_by(f.R, S)) %>%
  # Remove un-needed vars
  select(VisitID, Weights = weights) %>%
  # Join with Visits df
  right_join(visits, by = join_by(VisitID)) %>%
  # Re-factor Replicates
  mutate(Replicates = ifelse(Replicates > 2, "3+", Replicates) %>% as.ordered())
```
...
save_plot_list = function (ls) {
  ...
  iwalk(species, ~ ggsave(filename = file.path(dir.figs.ignore, "species eda", str_c(.x, ".png", sep = "")),
...
if (list.files(file.path(dir.figs.ignore, "species eda")) %>% length() == 0) {
...
knitr::include_graphics(file.path(dir.figs.ignore, "species eda", "Gadus chalcogrammus.png"))
```

### Starry Flounder {.unnumbered}

```{r echo=FALSE, out.width = '100%'}
knitr::include_graphics(file.path(dir.figs.ignore, "species eda", "Platichthys stellatus.png"))
```

### Great Sculpin {.unnumbered}

```{r echo=FALSE, out.width = '100%'}
knitr::include_graphics(file.path(dir.figs.ignore, "species eda", "Myoxocephalus polyacanthocephalus.png"))
```

### Pacific Herring {.unnumbered}

```{r echo=FALSE, out.width = '100%'}
knitr::include_graphics(file.path(dir.figs.ignore, "species eda", "Clupea pallasii.png"))
```

### Coho Salmon {.unnumbered}

```{r echo=FALSE, out.width = '100%'}
knitr::include_graphics(file.path(dir.figs.ignore, "species eda", "Oncorhynchus kisutch.png"))
```

### Dolly Varden {.unnumbered}

```{r echo=FALSE, out.width = '100%'}
knitr::include_graphics(file.path(dir.figs.ignore, "species eda", "Salvelinus malma.png"))
```

# Save data objects

```{r}
# Save "all" catch, events, visits objects
save(catch.all, events.all, visits.all, file = file.path(dir.data, "nfaa_alldata.rda"))

# Save some objects separately
saveRDS(events, file = file.path(dir.data, "nfaa_events.rds"))
saveRDS(abun, file = file.path(dir.data, "nfaa_abun.rds"))
saveRDS(diversity, file = file.path(dir.data, "nfaa_diversity.rds"))
saveRDS(size, file = file.path(dir.data, "nfaa_size.rds"))
saveRDS(sp.occur, file = file.path(dir.data, "nfaa_spoccur.rds"))

# Clean up obj name and remove working versions
catch = catch.2; rm(catch.0, catch.1, catch.2)
visits = visits.1; rm(visits.1)

# Clean up data ojbects except catch and visits
rm(list = ls()[!ls() %in% c('catch', 'visits')])

# Save cleaned up environment
save(list = ls(), file = file.path(here(), "data", "nfaa_2.rda"))
```
